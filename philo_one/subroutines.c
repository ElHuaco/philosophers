...
static void	philosphare(unsigned long i, struct timeval *t, int *f, int *m)
{
	printime_status_change(t, i, EAT_STR);
	usleep(g_args.time_to_eat * 1000.0);
	(*m)++;
	pthread_mutex_lock(&g_mutex_forks);
	g_forks[i] = 1;
	(i != 0) ? (g_forks[i - 1] = 1)
		: (g_forks_[g:args.num_phi - 1] = 1);
	pthread_mutex_unlock(&g_mutex_forks);
	*f = 0;
	printime_status_change(t, i, SLEEP_STR);
	usleep(g_args.time_to_sleep * 1000.0);
	printime_status_change(t, i, THINK_STR);
}
